...
Sans is a native macOS SwiftUI app (macOS 13+) that previews sample text across all installed font families in a grid. Zero external dependencies — Swift Package Manager with only system frameworks (SwiftUI, AppKit, CoreText).

**Source files** (all in `Sources/Sans/`):

- **FontApp.swift** — `@main` entry point, WindowGroup (1024×600 default)
- **ContentView.swift** — Main view: state management, search filtering with 150ms debounce, row grouping for the LazyVStack grid, dynamic height computation per row
...
- **FontManager.swift** — ObservableObject: enumerates system fonts on background thread, resolves fonts by CSS weight (maps NSFontManager 0-15 → CSS 100-900), caches resolved fonts keyed by `{family}-{weight}-{size}`
